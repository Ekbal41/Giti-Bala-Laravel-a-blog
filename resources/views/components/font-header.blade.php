    ...
    <div class="container flex items-center justify-center h-full max-w-6xl px-8 mx-auto sm:justify-between xl:px-0">
        ...
        <nav id="nav" class="absolute top-0 left-0 z-50 flex flex-col items-center justify-between hidden w-full h-64 pt-5 mt-24 text-sm text-gray-800 bg-white border-t border-gray-200 md:w-auto md:flex-row md:h-24 lg:text-base md:bg-transparent md:mt-0 md:border-none md:py-0 md:flex md:relative">
            ...
            <div class="flex flex-col block w-full font-medium border-t border-gray-200 md:hidden">
                ...
                @if (Auth::check())
                <a href="{{ route('dashboard') }}" class="w-full py-2 font-bold text-center text-pink-500">Dashboard</a>

                @else
                <a href="{{ route('login') }}" class="w-full py-2 font-bold text-center text-pink-500">Log Ini</a>
                @endif


                <a href="#_" class="relative inline-block w-full px-5 py-3 text-sm leading-none text-center text-white bg-indigo-700 fold-bold">Get
                    Started</a>
            </div>
        </nav>
        ...
        @if (Auth::check())
                <a href="{{ route('dashboard') }}" class="relative z-40 px-3 py-2 mr-0 text-sm font-bold text-pink-500 md:px-5 lg:text-white sm:mr-3 md:mt-0">Dashboard</a>
            ...
            <a href="#_" class="relative z-40 inline-block w-auto h-full px-5 py-3 text-sm font-bold leading-none text-white transition-all transition duration-100 duration-300 bg-indigo-700 rounded shadow-md fold-bold lg:bg-white lg:text-indigo-700 sm:w-full lg:shadow-none hover:shadow-xl">Get
                Started</a>
